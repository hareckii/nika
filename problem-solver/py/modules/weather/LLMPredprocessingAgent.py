...
class LLMPredprocessingAgent(ScAgentClassic):
    ...
    def on_event(
        ...
        self.logger.info(
            "LLMPredprocessingAgent finished %s",
    ...
    def run(self, action_node: ScAddr) -> ScResult:
        try:
            ...
            rrel_1 = ScKeynodes.resolve("rrel_1", sc_type.CONST_NODE_ROLE)
            rrel_2 = ScKeynodes.resolve("rrel_2", sc_type.CONST_NODE_ROLE)
            ...
            search_result_get_message = search_by_template(
                get_message_temp
            )[0]
            entity_node = search_result_get_message.get(_entity_node)
            message_type_node = search_result_get_message.get(
                _message_type_node
            )
            ...
            self.logger.info(
                f"Entity node valid: {entity_node.is_valid()}"
            )
            self.logger.info(
                f"Entity type: {get_elements_types(entity_node)}"
            )
            self.logger.info(
                f"Message type sys idtf: {get_element_system_identifier(message_type_node)}"
            )
            self.logger.info(
                f"Message sys idtf: {message_node.is_valid()}"
            )
            # -------------------------------------------------------------------
            ...
            search_result_get_llm_templates = search_by_template(
                get_llm_templates_temp
            )[0]
            input_json_node = search_result_get_llm_templates.get(
                _input_json
            )
            entity_template = search_result_get_llm_templates.get(
                _entity_template
            )
            prompt_json_node = search_result_get_llm_templates.get(
                _prompt_json
            )
            input_json = get_link_content_data(
                input_json_node
            )  # получаем из узла джейсончик номер раз в виде строки
            input_json_dict = self.str_to_dict(
                input_json
            )  # преобразуем его в словарь
            prompt_json = get_link_content_data(
                prompt_json_node
            )  #!!!!!!!!!!!!!!!!!!!!!! ПРОМТОВЫЙ  ДЖЕЙСОН В СТРОКОВОМ ВИДЕ
            ...
            search_result_get_entity_params = search_by_template(
                get_params_names_temp
            )
            ...
            for result in search_result_get_entity_params:
                ...
                self.logger.info(
                    get_element_system_identifier(result.get(_nrel_param))
                )
                self.logger.info(
                    get_element_system_identifier(result.get(_param))
                )
                # -------------------------------------------------------------------------------

            input_json_dict["question"] = get_link_content_data(question)
            input_json_dict["question_type"] = (
                get_element_system_identifier(message_type_node)
            )
            self.logger.info(f"Updated Input JSON:{input_json_dict}")

            some_link = generate_link(" ")
            if get_elements_types(some_link) == get_elements_types(
                entity_node
            ):
                ...
                concept_wit_entity = ScKeynodes.resolve(
                    "concept_wit_entity", sc_type.CONST_NODE_CLASS
                )
                lang_ru = ScKeynodes.resolve(
                    "lang_ru", sc_type.CONST_NODE_CLASS
                )
                ...
                entity_node = ScKeynodes.resolve(
                    entity_system_idtf, sc_type.CONST_NODE
                )
                # 4. Прикрепить линку как основной идентификатор
                generate_non_role_relation(
                    entity_node, entity_node_link, nrel_main_idtf
                )
                ...
                generate_connector(
                    sc_type.CONST_PERM_POS_ARC, lang_ru, entity_node_link
                )  # фокус покус
                # 6. Добавить в инпут
                input_json_dict["entity"] = entity_text
            else:
                self.logger.info("Entity type is NOT link")
                input_json_dict["entity"] = get_element_system_identifier(
                    entity_node
                )
            self.logger.info(f"Updated Input JSON:{input_json_dict}")
            ...
            for nrel_param in nrel_params:
                ...
                if result:
                    self.logger.info(
                        f"Nrel value found for {nrel_param}: {result[0]}"
                    )
                    answer = result[0].get(_answer)
                    input_json_dict[
                        get_element_system_identifier(nrel_param)
                    ] = get_link_content_data(answer)
                else:
                    self.logger.info(
                        f"Nrel value not found for {nrel_param}"
                    )
            self.logger.info("CHECK POINT №4")
            self.logger.info(f"Updated Input JSON:{input_json_dict}")
            input_json_str = self.dict_to_str(
                input_json_dict
            )  # !!!!!!!!!!!!!!!!!!!!ИНПУТОВЫЙ ДЖЕЙСОН УЖЕ В СТРОКОВОМ ВИДЕ
            # ---------------CHECK----------------------------------------------------------
            # отдаем джейсончики в модель
            self.logger.info("CHECK POINT №5")
            output_json_dict = self.get_llm_answer(
                json_template=json.loads(prompt_json),
                json_input=input_json_str,
            )
            self.logger.info(f"Output from LLM: {output_json_dict}")
            # находим новые значения параметров
            diff_params = self.find_new_params(
                input_json_dict, output_json_dict
            )
            ...
            reply_node = self.add_answer_to_result(
                message_node, answer_txt
            )
            generate_action_result(action_node, reply_node)
            return ScResult.OK
        except Exception as e:
            self.logger.exception(
                f"LLMPredprocessingAgent: finished with an error: {e}"
            )
    ...
    def get_llm_answer(
        self, json_template: dict, json_input: dict
    ) -> dict:
    ...
    def find_new_params(self, json_input: dict, json_output: dict) -> dict:
        ...
        diff = {}
        self.logger.info(
            f"json_output.keys() == json_input.keys(): {sorted(json_output.keys()) == sorted(json_input.keys())}"
        )
        if sorted(json_output.keys()) == sorted(json_input.keys()):
            for v1, v2 in zip(
                sorted(json_input.items()), sorted(json_output.items())
            ):
                if v1 != v2:
    ...
    def add_new_params_to_kb(self, new_params: dict, entity_node: ScAddr):
        ...
        self.logger.info(
            f"zip(new_links, list(new_params.keys())): {zip(new_links, list(new_params.keys()))}"
        )
        for link, key in zip(new_links, list(new_params.keys())):
            nrel = generate_non_role_relation(
                entity_node,
                link,
                ScKeynodes.resolve(key, sc_type.CONST_NODE_NON_ROLE),
            )
    ...
    def add_answer_to_result(self, message_addr: ScAddr, answer_text: str):
        ...
        answer_link = generate_link(
            answer_text, ScLinkContentType.STRING, sc_type.CONST_NODE_LINK
        )
        answer_link_node = generate_node(sc_type.CONST_NODE)
        # resolve or create all necessary nodes
        nrel_reply_node = ScKeynodes.resolve(
            "nrel_reply", sc_type.CONST_NODE_NON_ROLE
        )
        self.logger.info(
            f"nrel_reply node is valid: {nrel_reply_node.is_valid()}"
        )
        text_translation_node = ScKeynodes.resolve(
            "nrel_sc_text_translation", sc_type.CONST_NODE_NON_ROLE
        )
        self.logger.info(
            f"nrel_sc_text_translation is valid: {text_translation_node.is_valid()}"
        )
        reply_message_node = generate_node(sc_type.CONST_NODE)

        generate_non_role_relation(
            message_addr, reply_message_node, nrel_reply_node
        )
        generate_non_role_relation(
            answer_link_node, reply_message_node, text_translation_node
        )
        generate_connector(
            sc_type.CONST_PERM_POS_ARC, answer_link_node, answer_link
        )

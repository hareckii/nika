...
class IndexingAgent(ScAgentClassic):
    ...
    def run(self, action_node: ScAddr) -> ScResult:
        ...
        try:
            ...
            if not check_connector(
                sc_type.VAR_PERM_POS_ARC, message_type, message_addr,
            ):
                self.logger.info(
            ...
            if country_addr is not None:
                if not search_connector(
                    country_addr, city_addr, sc_type.VAR_COMMON_ARC,
                ):
            ...
            self.logger.info("WeatherAgent: finished with an error %s", e)
            return ScResult.ERROR

        entity_idtf = get_link_content_data(city_idtf_link)
        try:
            temperature = self.get_weather(entity_idtf, city_addr, country_addr)
    ...
    def get_weather(
        self, entity_idtf: ScAddr, city_addr: ScAddr, country_addr: ScAddr,
    ) -> float:
        # get entity longitude and latitude
        if country_addr is not None:
            country = get_link_content_data(
                self.get_ru_main_identifier(country_addr),
            )
            coordinates = requests.get(
                f"https://geocode.maps.co/search?city={entity_idtf}&country={country}",
            ).json()[0]
        else:
            coordinates = requests.get(
                f"https://geocode.maps.co/search?city={entity_idtf}",
            ).json()[0]
    ...
    def set_unknown_city_link(
        self, action_node: ScAddr, answer_phrase: ScAddr,
    ) -> None:
